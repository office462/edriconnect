...
🟢 ניהול תוכן = קישורים וקבצים שהבוט שולח (תשלום, סרטונים, שאלונים, הסכמים) + כרטיס QR לפוסט הרצאה
⚙️ הגדרות מערכת = מספר וואטסאפ של הבוט, שם קליניקה, הגדרות תצוגה
...
• פניות בתהליך עבודה
• פניות שהושלמו
• פירוט לפי סוגי שירות (ייעוץ, משפטי, הרצאות, מרפאה, פוסט הרצאה)
...
1. בחרי איש קשר מהרשימה — או לחצי "חדש" ליצירת איש קשר חדש ישירות מהטופס
2. בחרי סוג שירות
3. הפנייה תיווצר בסטטוס "ליד חדש"

💡 חדש! אפשר ליצור איש קשר חדש ישירות מתוך טופס פתיחת הפנייה — בלי לצאת לדף אנשי קשר.
...
מסלול פוסט הרצאה:
• PDF סיכום הרצאה (pdf) — נשלח למשתתפים אחרי ההרצאה
• קישור לספר (external_link, sub_type=book) — הצעה לרכישת הספר
• קישור לביקורת גוגל (external_link, sub_type=google_review) — בקשת המלצה

📌 במסלול "פוסט הרצאה" מופיע גם כרטיס QR/קישור וואטסאפ — קישור ישיר לבוט שמשתתפים סורקים בסוף ההרצאה.`
      },
      {
        id: 'sc-how-edit',
...
טאב "עיצוב ותצוגה":
• שם הקליניקה, שם הרופאה, תואר

טאב "הגדרות Flow":
• מספר וואטסאפ של הבוט — המספר שבו הבוט פועל. משמש ליצירת קישורי וואטסאפ ו-QR. חשוב לעדכן למספר האמיתי!
• הגדרות תהליך נוספות`
      },
      {
        id: 'ss-when',
        ...
        icon: ShieldCheck,
        color: 'text-gray-500',
        content: `במקרים הבאים:

• לעדכן מספר וואטסאפ של הבוט — אם המספר משתנה, עדכני בטאב "הגדרות Flow". זה ישפיע על כרטיס ה-QR בניהול תוכן ועל כל קישורי הוואטסאפ.
• לשנות שם קליניקה / שם רופאה / תואר — בטאב "עיצוב ותצוגה"
...
אם לא בטוחה אם צריך לשנות משהו כאן — שאלי לפני.`
      },
      {
        ...
        icon: Zap,
        color: 'text-gray-500',
        content: `שני דברים שמשפיעים ישירות:\n\n• מספר וואטסאפ של הבוט — קובע את הקישור וה-QR שמופיעים בניהול תוכן (מסלול פוסט הרצאה). אם המספר לא נכון, הקישור יוביל למקום הלא נכון.\n• שם קליניקה / רופאה / תואר — יכולים לשמש בהודעות הבוט.\n\n💡 כל שאר התוכן שהבוט שולח מנוהל דרך \"תוכן הבוט\" ו\"ניהול תוכן\".`
      }
    ]
  },
  {
    ...
    subs: [
      {
        ...
        content: `הבוט משלב בין 4 מקורות תוכן:

1. תוכן הבוט (תוכן הבוט במערכת) — הודעות טקסט כמו "אישור תשלום", "הנחיות הגעה", "בקשת מסמכים" וכו׳
2. ניהול תוכן (ניהול תוכן במערכת) — קישורי תשלום, סרטונים, שאלונים, הסכמים, PDF סיכום הרצאה
3. קטלוג הרצאות — פרטי הרצאות שהבוט מציג
4. הגדרות מערכת — מספר הוואטסאפ של הבוט, שם הקליניקה
...
מסלול חוות דעת משפטית:
• תשלום בהעברה בנקאית בלבד (לא Bit/Paybox)
• כשהפונה שולח אסמכתא — הבוט שולח לך התראה בווצאפ
• את מגיבה "כן" בווצאפ → הסטטוס משתנה אוטומטית ל"שולם"

מסלול קליניקה — שוכר ותיק:
• קוד סודי → שאלת "האם יש משהו נוסף?"
• אם כן → הפניה לשלום (מנהל הקליניקה) + הודעת WhatsApp אוטומטית לשלום

מסלול קליניקה — מתעניין חדש:
• בחירת חדר → צפייה → מידע+מחירון → תיאום → תשלום (Paybox+Bit) → אישור+PDF+goodbye

מסלול פוסט הרצאה (QR):
• PDF סיכום נשלח מיד ללא איסוף פרטים
• הזמנה לרשימת דיוור + איסוף פרטים
• אחרי 30 דקות ללא תגובה — המלצה אוטומטית על סדרת הרצאות + goodbye`
      },
      {
        id: 'wa-customer',
